    ...
    <div>
      ...
      <q-card-section class="flex flex-col gap-2 p-4">
        <div class="flex justify-between items-center">
          <h2 class="text-xl max-w-56 truncate font-semibold">{{ event.title }}</h2>
          <p class="text-lg font-semibold text-gray-700">{{ event.start_date }}</p>
        </div>
        ...
        <div class="mt-2 text-sm text-gray-500">
          ...
          <p><span class="font-semibold">Description:</span> {{ event.description }}</p>
            <a v-if="event.link" :href="event.link" target="_blank" class="text-primary font-semibold">See more</a>
        </div>
      </q-card-section>
    </div>
